...
#include <Wire.h>
#if defined(__AVR__) || defined(__i386__) || defined(ARDUINO_ARCH_SAMD) || defined(ESP8266) || defined(ARDUINO_ARCH_STM32) || defined(ARDUINO_ARCH_STM32F1)|| defined(ESP32)
 #define WIRE Wire
#else // Arduino Due
 #define WIRE Wire1
#endif
...
*/
/**************************************************************************/
Adafruit_PN532::Adafruit_PN532(uint8_t ss, SPIClass* hardwareSPI):
  _ss(ss),
  _clk(0),
  _mosi(0),
  _miso(0),
...
uint8_t Adafruit_PN532::spi_read(void) {
  ...
  if(_hardwareSPI) {
    // Hardware SPI read.
    x = _hardwareSPI->transfer(0x00);
  }
  else {
    // Software _hardwareSPI read.
    digitalWrite(_clk, HIGH);
